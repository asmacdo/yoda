...
\begin{itemize}[label=$\rightarrow$]
    ...
    \end{itemize}
    \item \item \textbf{MUST}: Environment definitions are version controlled (D.3).
    \begin{itemize}[label=$\square$]
    ...
    \begin{itemize}[label=$\square$]
        \item Are dependencies or container layers archived within the research object where possible (e.g., using
        Singularity SIF files, npm package-lock.json files)?
    \end{itemize}
\end{itemize}
