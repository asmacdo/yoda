...
% - A research object and its components can be shared, retrieved, and used by others
% - Introduce and connect the concept of a software "distribution"
% - More than "dumping scripts on Zenodo" — must be in a state consistent with reuse (#38)
% - S promises everything is within the boundary; D promises those references actually deliver
% - Yarik's STAMPED circle: someone else's D feeds your S (downloading containers, fetching data)
% - "Persistent" means future-proofed method of sharing (Zenodo, PyPI, conda-forge, DANDI) (Cody)
 % - Spectrum: minimum (publicly accessible with retrieval instructions) vs middle (all modules on persistent infrastructure, frozen versions, content-addressed) vs ideal (all external data mirrored, or zippep example RO Crate
% - Sharing vs Distributing distinction: sharing on GitHub with loose deps is not distributing. Distributing means packaged so it can be retrieved in the same state as intended.
% - The dev vs production analogy: P = pip install -e .; D = pip freeze / PyInstaller
