...
Actionability is the quality that elevates a research object from a static collection of metadata to an operational unit.
A research object is actionable when the procedures it describes can be carried out by following or executing its contents---not merely read about.
This ranges from well-documented manual steps that a human can follow to fully automated workflows that execute with a single command.

Actionability is cross-cutting: it applies to every other STAMPED dimension, distinguishing declarative metadata from operational capability.
A version-controlled dataset is more actionable when any version can be retrieved programmatically (e.g., \texttt{git checkout}, \texttt{datalad get}), not just identified by a label.
Provenance is more actionable when recorded commands can be re-executed (e.g., \texttt{datalad rerun}), not just inspected.
Modularity is more actionable when components can be composed and decomposed via tooling (e.g., \texttt{git submodule}), not just organized into directories.
Portability is more actionable when environments can be instantiated from a specification (e.g., \texttt{singularity run}), not just documented in a README.

At a minimum, a research object must contain sufficient instructions to reproduce all computational results.
Stronger forms of actionability replace documentation with executable specifications, reducing ambiguity and human error.
For example, a version control system can record not only that a file changed, but the exact command that produced the change, its inputs, and its outputs---making the commit itself re-executable.
Tools such as DataLad's \texttt{run} command demonstrate this pattern: the commit message becomes a machine-readable provenance record that can be replayed to regenerate results.
The principle is tool-agnostic: any system that captures executable provenance alongside version history moves the research object further along the actionability spectrum.

\subsection{Modularity}

\subsubsection{Motivation}

Research projects naturally encompass multiple stages---from study conception and raw data collection through analytics to publication.
Each stage produces artifacts that serve dual purposes: they are valuable independently for reuse in future work, yet must remain linked for full reproducibility of published results.

This mirrors good software development practices [cite software philosophy books].
When building software, developers maintain source code, build systems, and compilation environments separately from distributable binaries.
End users receive the compiled product, while developers retain the complete build toolchain.
Reliable and reproducible builds are essential in software distributions such as Debian's Reproducible Builds initiative~\cite{TODO}, ensuring that identical source code produces identical binaries across different build environments.
Similarly, reused components in software---such as dynamic libraries---exist as single, independently maintained copies to ease fixing, reduce duplication, and minimize storage requirements.

Module boundaries are pragmatically driven by what constitutes a ``sensibly useful independent unit.''
Most frequently, these boundaries align with derived products that add unique processing or analysis on top of their sources.
For instance, a processed neuroimaging dataset builds upon raw scanner data; a statistical analysis builds upon processed data; a publication builds upon analysis results.
Each layer can serve as a standalone module while preserving its connection to upstream dependencies.

Modularization also facilitates team collaboration in what has been termed ``SciOps''---the operational coordination of scientific workflows across team members.
Different researchers may contribute to data collection, preprocessing, statistical analysis, and visualization.
Clear module boundaries enable these handoffs: one researcher can complete preprocessing and version that module, while another begins downstream analysis without waiting for the entire project to stabilize.

Without modularity, monolithic structures present significant barriers to effective development, maintenance, and reuse.
Managing datasets as single, large units complicates incremental updates, obscures dependencies, inhibits flexible reuse or remixing of individual components, and is notably time-consuming.
...
Researchers naturally modularize their work using folders, but often lack formal guidance on organization and logistics to facilitate efficient project management and collaboration.
