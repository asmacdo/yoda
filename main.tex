...
Building on this foundation, we present the STAMPED properties (Self-contained, Tracked, Actionable, Modular, Provenance-recorded, Ephemeral, and Distributable) which characterize a well-formed reproducible research object.
Originating from the YODA Principles, STAMPED has been formalized as a framework for evaluating and improving the reproducibility of research objects.
Full reproducibility is an asymptotic goal.
STAMPED takes a pragmatic approach: each property can be satisfied at varying levels of rigor, allowing researchers to begin with practical, tool-native approaches and progressively strengthen them.
Research objects that adhere to these properties possess the necessary structural and operational foundations for reproducibility, substantially improving it in practice, even for complex usage scenarios.
...
\hline
\textbf{ID} & \textbf{Property} & \textbf{Principle} \\
\hline
S.1  & Self-contained & All assets essential to replicate computational execution MUST be contained within a single top-level research object \\
T.1  & Tracked        & Version information MUST be tracked for all assets \\
T.2  & Tracked        & All assets SHOULD be tracked using the same content-addressed version control system \\
A.1  & Actionable     & Research object MUST contain sufficient instructions to reproduce all computational results \\
M.1  & Modular        & Assets SHOULD be organized in a modular structure \\
M.2  & Modular        & Assets MAY be included directly or linked as subdatasets \\
P.1  & Provenance     & Provenance of all modifications MUST be annotated \\
P.2  & Provenance     & Code-driven provenance SHOULD be annotated programmatically, MUST include versions \\
E.1  & Ephemeral      & Computational results SHOULD be computed in ephemeral environments \\
D.1  & Distributable     & Computational environments MUST be explicitly specified \\
D.2  & Distributable     & Environment specifications SHOULD support reproducible builds \\
D.3  & Distributable     & Environment definitions MUST be version controlled \\
D.4  & Distributable     & Environment assets SHOULD be self-contained within the dataset \\
\hline
\end{tabular}
\end{table}
