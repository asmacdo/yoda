...
Currently it is most common for these aspects to be managed separately; this separation introduces complexity and undermines reproducibility, transparency, and rigor.

The Workflow Community Initiative’s FAIR Workflows Working Group (WCI-FW) defines a \textbf{workflow} as ``the formal specification of the data flow and execution control between executable components, the expected datasets, and parameter files.’’ % TODO: cite WCI-FW 2025
...
Many researchers already employ practices that support reproducibility, but lack a common framework to evaluate or communicate their approach.
Here we identify and codify these practices as the STAMPED properties (Self-contained, Tracked, Actionable, Modular, Portable, Ephemeral, and Distributable), originating from the YODA Principles, which characterize a well-formed reproducible research object.
Full reproducibility is an asymptotic goal;
STAMPED takes a pragmatic approach in which each property is a spectrum, from practical minimums---often what researchers are already doing---to aspirational ideals.
